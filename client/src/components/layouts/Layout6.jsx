...
const profile = {
  ...
  gallery: [
    ...
    'https://images.unsplash.com/photo-1544006659-f0b21884ce1d?w=600&auto=format&fit=crop',
  ],
  videos: [
  ...
  return (
    <>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 py-8 pt-28">
        <div className="text-center mb-12">
          ...
          <div className="flex justify-center gap-4 mt-6">
            {profile.socialLinks.map((social) => {
              ...
              return (
                <a key={social.label} href={social.url} target="_blank" rel="noreferrer" className="glassmorphism w-12 h-12 rounded-full border border-border/60 flex items-center justify-center hover:scale-110 transition-transform hover:border-primary/50">
        ...
        {/* Photo Gallery - Wide, two, wide, two squares pattern */}
        <div className="glassmorphism p-8 rounded-2xl mb-8 card-3d" style={{animation: 'gentleFloat 6s ease-in-out infinite', animationDelay: '2s'}}>
          ...
          <div className="grid grid-cols-6 gap-4 md:gap-6">
            {/* Row 1: wide rectangle */}
            {profile.gallery[0] && (
              <div className="col-span-6 rounded-xl overflow-hidden cursor-pointer hover:scale-105 transition-transform" onClick={() => openModal(profile.gallery[0], 'image')}>
            ...
            {/* Row 2: two horizontal rectangles */}
            {profile.gallery.slice(1, 3).map((src, i) => (
              <div key={`row2-${i}`} className="col-span-3 rounded-xl overflow-hidden cursor-pointer hover:scale-105 transition-transform" onClick={() => openModal(src, 'image')}>
                <div className="h-80 md:h-96 bg-gradient-to-br from-purple-500/20 to-blue-500/20">
            ...
            {profile.gallery[3] && (
              <div className="col-span-6 rounded-xl overflow-hidden cursor-pointer hover:scale-105 transition-transform" onClick={() => openModal(profile.gallery[3], 'image')}>
                <div className="h-48 md:h-56 bg-gradient-to-br from-purple-500/20 to-blue-500/20">
                  <img src={profile.gallery[3]} alt="Gallery 4" className="w-full h-full object-cover" />
                </div>
              </div>
            )}

            {/* Row 4: two horizontal rectangles */}
            {profile.gallery.slice(4, 6).map((src, i) => (
              <div key={`row4-${i}`} className="col-span-3 rounded-xl overflow-hidden cursor-pointer hover:scale-105 transition-transform" onClick={() => openModal(src, 'image')}>
                <div className="h-80 md:h-96 bg-gradient-to-br from-purple-500/20 to-blue-500/20">
                  <img src={src} alt={`Gallery ${i + 5}`} className="w-full h-full object-cover" />
                </div>
              </div>
            ))}
          </div>
            ...
            {profile.videos.map((src, i) => (
              <div key={i} className="rounded-xl overflow-hidden glassmorphism border border-border/60 cursor-pointer hover:scale-105 transition-transform" onClick={() => openModal(src, 'video')}>
                <div className="relative pt-[56.25%]">
                  <video src={src} className="absolute inset-0 w-full h-full object-cover" muted preload="metadata" />
        ...
        {/* Quick Actions (same as other layouts) */}
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="glassmorphism rounded-xl p-5 border border-border/60 hover:translate-y-[-3px] transition-transform">
              ...
              <div className="flex items-center gap-2 glassmorphism border border-border/60 rounded-lg px-3 py-2 justify-between">
                ...
                <button aria-label="Copy link" className="p-2 rounded-md hover:bg-muted/40" onClick={() => navigator.clipboard && navigator.clipboard.writeText('https://viioom.in/johndoe')}>
      ...
      {modalContent && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={closeModal}>
          <div className="relative max-w-4xl max-h-[90vh] mx-4" onClick={(e) => e.stopPropagation()}>
            <button onClick={closeModal} className="absolute -top-12 right-0 w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center hover:scale-105 transition-transform z-10">
              <X className="w-5 h-5 text-white" />
            </button>
            {modalType === 'image' ? (
              <img src={modalContent} alt="Full size" className="max-w-full max-h-full object-contain rounded-xl" />
            ) : (
              <video src={modalContent} controls autoPlay className="max-w-full max-h-full rounded-xl" />
            )}
          </div>
        </div>
      )}
